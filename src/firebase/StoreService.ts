...
class StoreService {
  public static collection(...pathSegments: string[]) {
    try {
      return collection(
        store,
        pathSegments.length > 1 ? createPath(...pathSegments) : pathSegments[0]
      );
    } catch (e: any) {
      console.log(e);
  ...
  //Without doc
  public static async arrayPush<T>(
    FieldValue: string,
    value: T,
  ...
  public static async removeArrayElement<T>(
    FieldValue: string,
  ...
  public static async setArray<T>(
    FieldValue: string,
    elements: T[],
    doc: DocumentReference<DocumentData>
  ) {
  ...
  public static async createDoc<T extends unknown>(
  ...
  public static async setDoc<T extends unknown>(
    data: WithFieldValue<T>,
